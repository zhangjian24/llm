...
const Sidebar: React.FC<SidebarProps> = ({ isOpen = true, onClose }) => {
  ...
  return (
    <div
      className={`fixed inset-y-0 left-0 w-64 sm:w-72 bg-white shadow-lg transform transition-transform duration-300 ease-in-out z-40 ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}
    >
      ...
      <div className="p-3 sm:p-4 border-t border-gray-200">
        <p className="text-xs sm:text-sm text-gray-500 text-center truncate px-2">
          Powered by Tongyi Qianwen AI
        </p>
      </div>
    </div>
  );
